...
$display("[SCB] FAIL addr=%0h exp=%0h got=%0h",tr.addr,ref_mem[tr.addr],tr.rdata);
end
end
end
endtask
...
$display("--------------------------------");
$display("PASS COUNT = %0d",env.scb.pass_count);
$display("FAIL COUNT = %0d",env.scb.fail_count);
$display("--------------------------------");
$finish;
...
apb_slave dut(.PCLK    (PCLK),.PRESETn (intf.PRESETn),
.PSEL    (intf.PSEL),
